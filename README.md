...
- **Actualización del cliente de escritorio verificada**: `/api/desktop/latest` publica el **SHA-256** del instalador y el cliente Electron lo **verifica antes de ejecutarlo** — evita RCE por un instalador manipulado (MITM/servidor comprometido). *(Requiere reconstruir el instalador para que la verificación quede activa en las estaciones.)*
